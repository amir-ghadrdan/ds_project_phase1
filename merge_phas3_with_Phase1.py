...
def tfidf_paragraf_docs():
    ...
    querylist = tokenizer(list_document_query[1])
    list_tfidf_paragraf = []
    list_tfidf_doc = []
    N = counter_paragraf()[0]
    tf_idf_query = TFIdf_compute_and_use_SequenceMatcher(set_list(tokenizer(list_document_query[1])), N, querylist)
...
def tf_idf_docANDquery():
    ...
    list_tf_idf_doc = []
    querylist = tokenizer(inputs_doc_query[1])
    dict_querylist = {}
    for i in querylist:
        dict_querylist[i] = 0
